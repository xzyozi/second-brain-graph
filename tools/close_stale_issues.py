...
def is_stale_issue(updated_at_str: str, days_threshold: int = 30, now_dt: Optional[datetime] = None) -> Tuple[bool, int]:
...
def close_issue(
    ...
    if dry_run:
        logger.info(f"[DRY-RUN] Would close Issue #{issue_number} (elapsed {elapsed_days} days): {comment}")
        return True
...
def process_stale_issues(
    ...
    for iss in issues:
        ...
        if is_stale:
            logger.info(f"Stale Issue #{num} '{title}' detected (inactive for {elapsed} days >= {days_threshold} days).")
...
def main() -> None:
    ...
    parser.add_argument("--days", type=int, default=30, help="Days threshold for inactivity (default: 30)")
