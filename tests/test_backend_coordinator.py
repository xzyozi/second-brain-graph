...
def test_coordinator_routing_ollama(
    ...
    with patch("tools.backend_coordinator.OllamaBackendAdapter") as mock_adapter_class:
        ...
        mock_adapter_class.assert_called_once_with(
            mock_get_config.return_value.profiles["coding_ollama"]
        )
...
def test_coordinator_routing_llama_server(
    ...
    with patch("tools.backend_coordinator.LlamaServerBackendAdapter") as mock_adapter_class:
        ...
        # Verify adapter called
        mock_adapter_class.assert_called_once_with(
            mock_get_config.return_value.profiles["reasoning_economy"], mock_get_config.return_value
        )
        mock_adapter_instance.execute.assert_called_once_with(request)
...
def test_coordinator_gpu_lease_timeout(
    ...
    mock_lease.__enter__.side_effect = TimeoutError(
        "Failed to acquire GPU lease within 60 seconds."
    )
