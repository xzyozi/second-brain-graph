...
また、LLM ノードのシステムプロンプトが Python コード内にハードコードされており、保守性・他プロジェクトへの適応性の課題となっていました。

本機能により、設定ファイルを **YAML 形式（コメント記述可能）** に統一し、以下の 2 領域に責務を分離した共通設定基盤を構築しました。
...
| :--- | :--- | :--- |
| **`config/models.yaml`** | ・モデルハイパーパラメータ (`temperature`, `max_tokens`)<br>・Aider 自動編集エンジン設定 (`timeout`, `edit_format`)<br>・GPU 排他リース (`mode: exclusive`)<br>・バックエンドプロファイル (`ollama` / `llama_server`) | ・各モデルの推奨コンテキスト長や VRAM 消費量の根拠をコメントとして永続化<br>・`fallback: disabled` による勝手な代替フォールバックを排除した厳格実行 |
...
from tools.config_loader import (
    get_config,  # 統合 AppConfig (models, prompt)
...
- **Fail-fast 設計**: 設定ファイルが存在しない場合やバリデーション失敗時は暗黙のフォールバックを行わず即座に例外を送出します。

---

## 4. 検証実績 (`tests/test_config_loader.py`)

以下の単体テストにより、YAML ロードおよび Pydantic バリデーションの完全性を保証しています（全 15 ケース合格）：

1. `test_load_model_config_loads_yaml`: `models.yaml` の妥当性検証
2. `test_load_prompt_config_loads_yaml`: `prompt.yaml` の各ロールプロンプト取得検証
3. `test_get_config_returns_unified_app_config`: 統合 `AppConfig` の取得検証
4. `test_profile_*`: 不正なポート、欠損エンドポイントの拒否検証 (Fail-closed)
5. `test_backend_config_*`: ルーティング不整合および未定義プロファイル参照の拒否検証
