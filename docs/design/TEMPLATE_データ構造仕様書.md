...
| 項目 | 内容 |
| :--- | :--- |
| 文書番号 | SBOS-DS-[NUMBER] （例: SBOS-DD-007） |
| 版数 | Rev.1.0（新規作成） |
| 改訂日 | YYYY年MM月DD日 |
| 作成日 | YYYY年MM月DD日 |
| 関連文書 | SBOS-BD-002（基本設計書）、SBOS-DD-003（詳細設計書正本） |
| 対象読者 | 開発実装エンジニア / データ設計者 / 運用エンジニア / AIエージェント |
...
### 2.2 データ構造例（スキーマ説明）
各データ要素がどのようなネスト関係やキー構造を持つかを、構造要素の役割とともに定義する。

- **ルートオブジェクト**: 対象タスクまたはプロジェクトの識別キー
- **ステータス属性**: 現在の実行状態を示す状態タクソノミー文字列
- **試行カウンター**: 現在の再試行周回数および最大試行上限値
- **エラー属性**: 直近の失敗原因を示すエラーカテゴリ分類
- **タイムスタンプ**: 最終更新日時を示す日時文字列
