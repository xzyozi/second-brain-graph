# 課題・矛盾点一覧 (Problem Management) Rev.2.12
...
| **PM-037** | DD-003 / OP-001 | 🔴 高 | 衛星リポジトリの「排他制御（ロック機構）」の欠如。複数タスク同時実行時のワーキングツリー競合・破壊リスク | `plan_node` 前に `metadata/projects/<PROJECT_KEY>/.lock` による排他制御（ブロック/キューイング）機構を導入 | 🟡 未解決・タスク化 |
| **PM-038** | DD-003 / OP-001 / MULTI-001 | 🔴 高 | `tasks.md` へのシステム状態埋め込み（HTMLコメント）による脆弱性。人間の誤編集でシステムが状態（B7等）を見失うリスク | システム状態（ラウンド数など）を `metadata/projects/<PROJECT_KEY>/state.json` へ分離し SSOT 化 | 🟡 未解決・タスク化 |
...
| **PM-041** | 複数設計書横断 | 🔴 高 | 台帳・メタデータの正本パスが二重定義（`metadata/` vs 衛星直下） | `metadata/` を唯一の正本として文書体系全体で統一し、旧配置は移行情報としても明示的に廃止する | 🟡 未解決・タスク化 |
| **PM-042** | DD-003 / ORCH | 🔴 高 | B7ブロッカー判定の履歴スキーマが新旧設計書間で非互換（安全回路不成立） | B7の判定基準・履歴項目・最終状態を詳細設計書の定義へ一本化する | 🟡 未解決・タスク化 |
| **PM-043** | ORCH / DD | 🔴 高 | LLMタイムアウト・システム例外時の状態遷移契約の不足 | 例外種別ごとに検知箇所、再試行上限、待機方針、終了状態、通知・履歴を状態遷移表で定義する | 🟡 未解決・タスク化 |
| **PM-044** | OP-001 / ORCH | 🔴 高 | 破壊的なGit復旧に対する保全・復元方針の欠如 | 作業専用worktree/ブランチを前提とし、削除前退避、dry-run、失敗時は中止する方針を設計する | 🟡 未解決・タスク化 |
| **PM-045** | ORCH / BD | 🔴 高 | 自動ブランチ・PR運用が正常系のみで異常系状態の定義不足 | 既存ブランチ/PR、dirty tree等の事前検証、失敗時の冪等性、人間への引継ぎを明文化する | 🟡 未解決・タスク化 |
| **PM-046** | DD / ORCH | 🔴 高 | 同一衛星への並行実行を扱う排他制御の契約未完成（PM-037関連） | グラフ開始前の必須処理としてロック取得失敗時の待機/中止、異常終了・監査ログまで定義する | 🟡 未解決・タスク化 |
| **PM-047** | ENV-001 / DD | 🟠 中 | モデル設定SSOTとフォールバック方針の矛盾 | 設定欠損時の振る舞いを明文化し、モデル値の管理先を一箇所に固定する | 🟡 未解決・タスク化 |
| **PM-048** | 全文書横断 | 🟠 中 | 文書間の版数参照が現行版と不一致 | ヘッダー・関連文書を一括更新し、本文では固定版数より文書番号参照を優先するよう改修 | 🟡 未解決・タスク化 |
| **PM-049** | ORCH | 🟠 中 | 旧ORCH文書の位置付けが不明瞭（現行仕様との混同リスク） | 現行設計と旧設計を明確に分離し、旧仕様は非規範と表示。可能なら docs/archive/ へ移動する | 🟡 未解決・タスク化 |
...
## 4. 改訂履歴
- **2026/07/30 (Rev.2.12)**: 設計文書レビューに基づく9件の重要指摘（PM-041〜PM-049: 台帳正本一本化、B7安全回路、例外状態遷移、Git保全等）を新規追加。
