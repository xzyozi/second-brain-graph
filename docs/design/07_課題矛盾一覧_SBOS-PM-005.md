# 課題・矛盾点一覧 (Problem Management) Rev.2.10
...
* **課題**: 人間の可読性とシステム状態（State）の管理を一つのMarkdownファイルに相乗りさせている点に運用上のリスクがある。B7ブロッカーの判定やリトライ回数の状態を `tasks.md` 内のHTMLコメントとして書き込みスクリプトでパースしているが、人間が誤ってコメント行を消したりフォーマットを崩すと、システムがB7状態を見失い致命的な動作不良を起こす。
* **解決案**: `tasks.md` はあくまで人間の閲覧・起票用とし、システムの実行状態やラウンド数は `metadata/projects/<PROJECT_KEY>/state.json` のような機械専用のJSONに分離し、そこを正（SSOT）として参照する設計へ変更する。
